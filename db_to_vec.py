...
if __name__ == '__main__':
    ...
    # get all abstracts
    entrys = db.session.query(PaperEntry.title,PaperEntry.abstract,PaperEntry.abstract_processed,PaperEntry.bibcode).all()
    ...
    if not all(processed_abstracts):

        # tokenize text
        for i in tqdm(range(len(abstracts))):

            # process if need be, takes ~1-2 hours for 1.1M abstracts
            ...
            # TODO commit to db every 1000 entries
    ...
    # ran out of ram on my laptop, need to delete some unused variables
    del processed_abstracts
