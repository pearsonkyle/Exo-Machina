...
# comment out categories to reduce size of database
category_map = {
...
# description: https://arxiv.org/category_taxonomy
...
if __name__ == "__main__":
    ...
    with open(arxiv, 'r') as f:
        ...
        for i in tqdm(range(len(lines))):
            ...
            entry.title = clean_text(data['title'])
            entry.abstract = clean_text(data['abstract'])
            entry.pub = data['journal-ref']

            try:
                entry.year = int(data['journal-ref'].split(',')[-1])
            except:
                entry.year = int(data['versions'][-1]['created'].split(' ')[3])

            # check if categories are in map
            entry.categories = ""
            keep_entry = False
            for cat in data['categories'].split(' '):
                if cat in category_map:
                    entry.categories += category_map[cat] + ','
                    keep_entry = True

            # skip entry is category is not in map
            if not keep_entry:
                continue
